...
> 0,1사이의 값을 다음 층에 전달해줌. 중첩이 되면 선형의 형태를 띄게 된다 !!!
>-1~1 부분을 보면 선형의 그래프를 띈다...  기울기가 고정된다.
...
> ocr에 많이 쓰임. 문자인식에 렐루보다 많이 씀. -1 이라는 값이 나온다는 특성 떄문.
> 딥뉴런 / cnn / rnn 3가지가 있는데
> 우리가 계속 접해온건 dnn. 그다음엔 cnn. 그다음엔 rnn.
> 퍼셉트론들이 연결되어있는 구조 자체가 다르다.
...
> C는 상황에 따라 작은 값으로 변경 가능
> C'를 음수로 설정하기도 함(C를 분수로 설정하는 것. 0.01같은, 그러면 C'가 음수가 된다).
> 똑같은 C'를 빼도 똑같으니까....exp(x) 값을 애초에 줄이는 것.
> C의 목적 : 위 아래 값을 줄여서 오버플로우를 안나게 하겠다 !
